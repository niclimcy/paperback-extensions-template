...
import {
    ...
    resetSettings,
    uploadersSettings
} from './PaperbackSettings'

import {
    ChapterList,
    CMLanguages
} from './PaperbackHelper'

const PAPERBACK_DOMAIN = 'https://paperback-ios.github.io/paperback-extensions-template'
const PAPERBACK_API = 'https://paperback-ios.github.io/paperback-extensions-template/api'
const LIMIT = 300
...
export const PaperbackInfo: SourceInfo = {
    ...
    description: 'Extension that pulls manga from paperback-ios.github.io',
    contentRating: ContentRating.EVERYONE,
    websiteBaseURL: PAPERBACK_DOMAIN,
...
export class Paperback implements MangaProviding, ChapterProviding, SearchResultsProviding, HomePageSectionsProviding {
    ...
    getMangaShareUrl(mangaId: string): string { return `${PAPERBACK_DOMAIN}/comic/${mangaId}` }

    async getMangaDetails(mangaId: string): Promise<SourceManga> {
        const request = App.createRequest({
            url: `${PAPERBACK_API}/comic/${mangaId}`,
            method: 'GET'
        })
    ...
    async createChapterRequest(mangaId: string, page: number): Promise<ChapterList> {
        ...
        const request = App.createRequest({
            url: `${PAPERBACK_API}/comic/${mangaId}/chapters?page=${page}&limit=${LIMIT}${!Languages.includes('all') ? `&lang=${Languages}` : ''}`,
            method: 'GET'
        })
        const response = await this.requestManager.schedule(request, 1)
        this.CloudFlareError(response.status)
    ...
    async getChapterDetails(mangaId: string, chapterId: string): Promise<ChapterDetails> {
        const request = App.createRequest({
            url: `${PAPERBACK_API}/chapter/${chapterId}/`,
            method: 'GET'
        })
        const response = await this.requestManager.schedule(request, 1)
        this.CloudFlareError(response.status)
    ...
    async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
        ...
        const createSectionRequest = (sort: string) => ({
            ...
            request: App.createRequest({
                url: `${PAPERBACK_API}/v1.0/search/&page=1&limit=${LIMIT}&sort=${sort}&t=false`,
                method: 'GET'
            })
        })
    ...
    async getViewMoreItems(homepageSectionId: string, metadata: any): Promise<PagedResults> {
        ...
        switch (homepageSectionId) {
            case 'view':
                param = `/v1.0/search/?page=${page}&limit=${LIMIT}&sort=view&t=false`
                break
            case 'follow':
                param = `/v1.0/search/?page=${page}&limit=${LIMIT}&sort=follow&t=false`
                break
            case 'uploaded':
                param = `/v1.0/search/?page=${page}&limit=${LIMIT}&sort=uploaded&t=false`
                break
            default:
    ...
    async getSearchResults(query: SearchRequest, metadata: any): Promise<PagedResults> {
        ...
        if (query.title) {
            request = App.createRequest({
                url: `${PAPERBACK_API}/v1.0/search?q=${query.title.replace(/ /g, '%20')}&limit=${LIMIT}&page=${page}`,
                method: 'GET'
            })
        } else {
            request = App.createRequest({
                url: `${PAPERBACK_API}/v1.0/search?page=${page}&limit=${LIMIT}${includedGenres.length > 0 ? includedGenres.join('') : ''}${excludedGenres.length > 0 ? excludedGenres.join('') : ''}${Sort.length > 0 ? Sort.join('') : ''}${CreatedAt.length > 0 ? CreatedAt.join('') : ''}${Type.length > 0 ? Type.join('') : ''}${Demographic.length > 0 ? Demographic.join('') : ''}`,
                method: 'GET'
            })
        }
        const response = await this.requestManager.schedule(request, 1)
    ...
    CloudFlareError(status: number): void {
        if (status == 503 || status == 403) {
            throw new Error(`CLOUDFLARE BYPASS ERROR:\nPlease go to the homepage of <${PaperbackInfo.name}> and press the cloud icon.`)
        }
    }

    async getCloudflareBypassRequestAsync(): Promise<Request> {
        return App.createRequest({
            url: PAPERBACK_DOMAIN,
            method: 'GET',
            headers: {
                'referer': `${PAPERBACK_DOMAIN}/`,
                'user-agent': await this.requestManager.getDefaultUserAgent()
